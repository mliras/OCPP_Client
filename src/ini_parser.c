...
int writeToDisk(char *filename)
{
	 ...
	 //Pag 44: When a chargepoint or a connector is set to status unavailable by a change availability command, the 'unavailable' status MUST be persistent
	 //across reboots. The
...
int readFromDisk(char *filename)
{
	 ...
	 while (!ferror(file) && i<tmp)
	 {
	 	 ...
	 	 if (debug) printf("LOS VALORES LEIDOS DE DISCO SON %f", connectorValues[i]);
	 	 i=i+1;
	 }
