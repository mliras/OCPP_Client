...
int main(int argc, char *argv[])
{
	if (argc<2)
	{
		printf ("Syntax Error: %s <conector> <Lock Status (0 for unlocked)>", argv[0]);
		exit (-3);
	}
...
printf("Se pondra el conectoir %d a %d", connector, valor);
	if (connector==1) datos->cable1locked=valor;
	if (connector==2) datos->cable2locked=valor;
