...
double getCurrentMeterValueOfConnector(int connector)
{
	return connectorValues[connector];
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getVendor()
{
	return getConfigurationKeyStringValue("ChargePointVendor");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getModel()
{
	return getConfigurationKeyStringValue("ChargePointModel");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargePointSN()
{
	return getConfigurationKeyStringValue("ChargePointSerialNumber");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargeBoxSN()
{
	return getConfigurationKeyStringValue("ChargeBoxSerialNumber");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getFirmwareVersion()
{
	return getConfigurationKeyStringValue("FirmwareVersion");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargePointICCID()
{
	return getConfigurationKeyStringValue("ChargePointICCID");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargePointIMSI()
{
	return getConfigurationKeyStringValue("ChargePointIMSI");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargePointMeterType()
{
	return getConfigurationKeyStringValue("getChargePointMeterType");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargePointMeterSerialNumber()
{
	return getConfigurationKeyStringValue("ChargePointMeterSerialNumber");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargeBoxIP()
{
	return getConfigurationKeyStringValue("ChargeBoxIP");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargeBoxPort()
{
	return getConfigurationKeyStringValue("ChargeBoxPort");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargeBoxURL()
{
	return getConfigurationKeyStringValue("ChargeBoxURL");
}

//TODA ESTA INFORMACION ESTA ESPECIFICADA EN EL PUNTO 4.2 (PAG 34)
const char *getChargeBoxID()
{
	return getConfigurationKeyStringValue("ChargeBoxID");
}
...
{
	if (conn<0) return -1;
	if ((currentConnectorErrorStatus[conn]<0)||(currentConnectorErrorStatus[conn]>20)) return -1;

	if (currentConnectorErrorStatus[conn]==_CP_ERROR_NOERROR) return 0;
	else return currentConnectorErrorStatus[conn];
}

//Conn debe ser un valor de 0 a N-1
void setErrorStateOfConnector(int conn, int error)
{
	if (conn<0 || conn>NUM_CONNECTORS) return;
	if ((currentConnectorErrorStatus[conn]<0) ||(currentConnectorErrorStatus[conn]>20)) return ;

	currentConnectorErrorStatus[conn]=error;
}
...
int modifyConfigurationKey(char *key, char *value)
{
	...
	//		    		printf("\nINT VALUE ES: %d",getConfigurationKeyIntValue("LocalAuthListEnabled"));
			    		return 0;
			    	}
			    }
			    ...
			    if (getConfigurationKeyType_i(location)==2)
			    {
			    	...
			    	if (configurationValues[location].stringValue)
			   // 		printf("\nVALUE ES: %s",configurationValues[location].stringValue);
...
//Esta funcion es la encargada de discernir de adonde se enviaran los logs (FICHERO, BD, GUI, SCREEN)
void addLog(char *texto, char *arg1, char *arg2, int warning_level, enum logtype log_to)
{
	if (!texto) return;



	int max_log=LOG_ERROR;
	max_log=getConfigurationKeyIntValue("LogLevel");

	//NO_LOG. Salimos
	if (max_log==0) return;

	//Si el max log level es ERROR y NOS LLEGA un INFO o un DEBUG (menores), salimos
	if (warning_level<max_log) return;

	//Por defecto logará en BD
	if (log_to==ANY)
	{
		char *adonde=getConfigurationKeyStringValue("LogFile");
		if (adonde)
		{
			if (strcmp(adonde,"DB")==0) log_to=DB;
			else if (strcmp(adonde,"SCREEN")==0) log_to=SCREEN;
			else log_to=FICHERO;
		}
	}

	//Como al final no va a tener GUI, lo logamos todo a BD
	if (log_to==GUI) log_to=DB;
	//if (!guiready && log_to==GUI) log_to=SCREEN;

	//Y si la base de datos no esta lista, se utilizan ficheros
	if (!isSQLiteConnected()) log_to=FICHERO;



	//Calculamos la longitud de la cadena de texto final de entrada
	int longitud=strlen(texto)+1;
	if (arg1) longitud=longitud+strlen(arg1)+2;
	if (arg2) longitud=longitud+strlen(arg2)+2;

	char *currentTime=getCurrentTime();

	int longitud2;
	if (currentTime) longitud2=longitud+strlen(currentTime)+2+10;//los 10 son por la cadena '[WARNING]\0', el resto son mas cortas

	//Creamos esa nueva cadena
	char *mensaje=(char *)calloc(1, 512);
	if (arg1 && arg2) sprintf(mensaje, texto,arg1, arg2);
	else if (arg1) sprintf(mensaje, texto,arg1);
	else if (arg2) sprintf(mensaje, texto,arg2);
	else strcpy(mensaje, texto);

	char *mensaje2=(char *)calloc(1, 1024);
	sprintf(mensaje2, "[%s]",currentTime);

	if (warning_level==LOG_ERROR) strcat(mensaje2, "[ERROR] ");
	else if (warning_level==LOG_INFO) strcat(mensaje2, "[INFO] ");
	else if (warning_level==LOG_DEBUG) strcat(mensaje2, "[DEBUG] ");
	else if (warning_level==LOG_WARNING) strcat(mensaje2, "[WARNING] ");

	strcat(mensaje2,mensaje);

	while(logMutex) usleep(50);

//	if (debug) printf("LOGAMOS: %s", mensaje2);

	logMutex=1;
	if (log_to==FICHERO)
	{
		writeLogToFile(mensaje2, warning_level);
	}
	if (log_to==DB)
	{
		insertSQLiteLog(mensaje2, warning_level);
	}
	else if (log_to==SCREEN)
	{
		printf("\n%s", mensaje2);
	}
	else if (log_to==GUI)
	{
		//Comentamos esta Linea porque ya no tenemos GUI en el ChargePoint
		//printLogToGUI(mensaje2, warning_level);
	}

	//free(texto);
	if (mensaje) free(mensaje);
	if (mensaje2) free(mensaje2);
	logMutex=0;
}
...
//Esta funcion para todas las transacciones. Pone cada conector a UNAVAILABLE y luego "instala" el firmware.
//Es llamada por firmwareUpdate
int installFirmware()
{
	//Cuando vayamos a instalar el firmware... primeor ponemos el chargepoint unavailable porque en la pag 44 pone:
	//Pag 44: The charge point may use the unavailable status for other purposes like while updating firmware.
	chargePointStatus=_CP_STATUS_UNAVAILABLE;

	//...luego recorremos los conectores y vamos parando las transacciones
	for (int conector=0; conector<NUM_CONNECTORS; conector++)
	{
		//... y poniendo los conectores unavailable
		changeConnectorStatusWithErroCode(conector, _CP_STATUS_UNAVAILABLE,_CP_ERROR_INTERNALERROR, "Installing firmware"); //<-- Pag 42

		//...si algun conector esta inmerso en una transaccion, paramos la transaccion
		if (isCurrentlyUnderTransaction(conector))
		{
			//...y para ello mandamos antes un send stop trasnsaction request
			double meterStopValue=connectorValues[conector];

			int transactionId=getTransactionId(conector);

			//Paramos la transaccion
			stopTransaction(conector);

			//Ponemos el conector a NO DISPONIBLE
			changeConnectorStatus(conector, _CP_STATUS_UNAVAILABLE);

			//Mandamos el mensaje
			send_stoptransaction_request(meterStopValue, transactionId, _ST_REBOOT, NULL);
		}
	}

	////////////////////////////////////////////
	//INSTALAMOS EL FIRMWARE <-- NO IMPLEMENTADO
	////////////////////////////////////////////

	chargePointStatus=_CP_STATUS_AVAILABLE;
	changeConnectorStatus(0, _CP_STATUS_AVAILABLE);

	return 1; //<-- NOT IMPLEMENTED. SIEMPRE DEVUELVO 1 (SI FALLA LA ACTUALIZACION DEBERIA DEVOLVER 0)
}
...
int authorize(const char *miidtag)
{
...
// The	LocalAuthorizeOffline  configuration  key  controls  whether  a  Charge  Point  will  authorize
	...
	//char *miidtag=strdup(idtag);
	addLog("Trying to get the authorization status for %s", miidtag, NULL, LOG_DEBUG, ANY);
	if (isCPOffline())
	{
		...
		if (getConfigurationKeyIntValue("LocalAuthorizeOffline"))
		{
			...
			{
				if (getAuthorizationListEntryStatus(miidtag)>=0)
				{
					int st=checkValidAuthorizationListEntry(miidtag);
					if (st) addLog("With CP Offline, Authorization List is enabled and idtag %s is in the authorization List and is Accepted", miidtag, NULL, LOG_DEBUG, ANY);
					else addLog("With CP Offline, Authorization List is enabled and idtag %s is in the authorization List and is REJECTED", miidtag, NULL, LOG_ERROR, ANY);
					//addSendAuthorizeRequestWhenOnline()  <-- NOT IMPLEMENTED: Cuando se restaure la conexion, hay que mandar un sendAuthorize

					//st es 1 si es accepted o concurrent
					return st;
				}
				else
				{
					//Si no lo encuentra lo intentamos con la cache (Mas abajo)
				}
			}
			...
			if (isAuthorizationCacheEnabled())
			{
				...
				if (getAuthorizationCacheEntryStatus(miidtag)>=0)
				{
					int st=checkValidAuthorizationCacheEntry(miidtag);
					if (st) addLog("With CP Offline, Authorization Cache is enabled and idtag %s is in the authorization Cache and is Accepted", miidtag, NULL, LOG_DEBUG, ANY);
					else if (st) addLog("With CP Offline, Authorization Cache is enabled and idtag %s is in the authorization Cache and is REJECTED", miidtag, NULL, LOG_ERROR, ANY);
		...
		{
			addLog("With CP Offline, LocalAuthorizeOffline is not active. Denying Log for %s", miidtag, NULL, LOG_DEBUG, ANY);
			//return _CP_AUTHORIZATION_INVALID;
			return 0;
		}
	}
	else
	...
	//		show_authorization_cache();
			...
			if (isAuthorizationListEnabled())
			{
				...
				{
					//no esta en la lista.
					send_authorize_request(miidtag);

					addLog("With CP Online, Authorization List is enabled and idtag %s is NOT in the authorization List, but LocalPreauthorize is active", miidtag, NULL, LOG_DEBUG, ANY);
				...
				{
					int st=checkValidAuthorizationListEntry(miidtag);
					if (st) addLog("With CP Online, Authorization List is enabled and idtag %s is in the authorization List and is Accepted", miidtag, convert(st), LOG_DEBUG, ANY);
					else  addLog("With CP Online, Authorization List is enabled and idtag %s is in the authorization List and is REJECTED", miidtag, convert(st), LOG_ERROR, ANY);
					//devuelve 0 o 1 si esta ACCEPTED o CONCURRENT_TX
					return st;
				}
			}
			...
			if (isAuthorizationCacheEnabled())
			{
				if (getAuthorizationListEntryStatus(miidtag)<0)
				{
					addLog("With CP Online, Authorization Cache is enabled and idtag %s is NOT in the authorization Cache, but LocalPreauthorize is active", miidtag, NULL, LOG_DEBUG, ANY);
				...
				{
					int st=checkValidAuthorizationCacheEntry(miidtag);
					if (st) addLog("With CP Online, Authorization Cache is enabled and idtag %s is in the authorization Cache and is Accepted", miidtag, convert(st), LOG_DEBUG, ANY);
					else if (st) addLog("With CP Online, Authorization List is enabled and idtag %s is in the authorization List and is REJECTED", miidtag, convert(st), LOG_ERROR, ANY);
		...
		else
		{
			...
			if (isAuthorizationListEnabled())
			{
				if (getAuthorizationListEntryStatus(miidtag)<0)
				{
					addLog("With CP Online, and Localeauthorization Offline, and Authorization List active and idtag %s NOT in the authorization List. A message needs to be sent to CS", miidtag, NULL, LOG_DEBUG, ANY);
				...
				{
					addLog("With CP Online, and Localeauthorization Offline, and Authorization List active and idtag %s IN the authorization List.", miidtag, NULL, LOG_DEBUG, ANY);

					//devuelve 0 o 1 si esta ACCEPTED o CONCURRENT_TX
					return(checkValidAuthorizationListEntry(miidtag));
				}
			}
			...
			if (isAuthorizationCacheEnabled())
			{
				if (getAuthorizationListEntryStatus(miidtag)<0)
				{
					addLog("With CP Online, and Localeauthorization Offline, and Authorization Cache active and idtag %s IN the authorization Cache.", miidtag, NULL, LOG_DEBUG, ANY);
					send_authorize_request(miidtag);

					//Lo rechazamos temporalmente
					return 0;
				}
				else
				{
					addLog("With CP Online, and Localeauthorization Offline, and Authorization Cache active and idtag %s IN the authorization Cache.", miidtag, NULL, LOG_DEBUG, ANY);
					return checkValidAuthorizationCacheEntry(miidtag);
				}
			}
...
void changeConnectorStatus(int connector, int status)
{
	...
	if (_REGISTRATIONSTATUS==_RS_NOTCONNECTED) sendConnectorStatusWhenOnline=1;

	int errorCode=_CP_ERROR_NOERROR; //<-NOT IMPLEMENTED
...
void changeConnectorStatusWithErroCode(int connector, int status, int ErrorCode, char *extraInfo)
{
	connectorStatus[connector]=status;
...
void showReservations2()
{
	...
	puts("\n===============================\nPermisos de reservas en conectores: ");
	for (int i=0; i<NUM_CONNECTORS; i++)
	{
		printf("\nConector %d: ", i+1);
...
//conn es un valor de 1 a N, porque si es 0, va a buscar por todos los conectores
int getReservationId(int conn)
{
	...
	if (conn>0)
	{
		if (connectorReservations[conn-1])
		{
			return connectorReservations[conn-1]->reservationId;
		}
...
//Conn debe ser un valor de 0 a n-1
char* getReservationIdTag(int conn)
{
	if (conn>0)
	{
		if (connectorReservations[conn])
		{
				return connectorReservations[conn]->idTag;
		}
...
int removeReservation(int reservationId)
{
	...
	//También la cerramos en el SQLite
	closeReservationInSQLite(reservationId);
...
int addReservation(int ConnectorId, char *expiryDate, char *idTag, int reservationId, const char *parentIdTag)
{
	...
	//Lo metemos en SQLite
	insertReservationInSQLite(reservationId, ConnectorId+1, expiryDate, idTag);
...
//Connector debe ser un valor de 0 a N-1. Devuelve 0 o 1
		...
		const char *expiryDate_str, int transactionId)
{
	...
	transaccion->currentDelivered=0.0;
	transaccion->chargingProfile=currentChargingProfiles[connectorId-1];

	transactions[connectorId-1]=transaccion;

	//ESTO LO MUESTRA POR PANTALLA
//	if (debug) showTransactions();

	//ACTUALIZAMOS EL SQLITE
	insertTransactionInSQLite(transactionId, connectorId, timeStamp, NULL, idTag, 0);

	//LO QUITAMOS PORQUE YA NO TENEMOS GUI
...
int removeTransaction(int connectorId)
{
	...
	{
		int transactionId=transactions[connectorId]->transactionId;

		freeTransaction(transactions[connectorId]);
		transactions[connectorId]=NULL;

		//Stop Transaction in SQLITE
		char *stopTime=getCurrentTime();
		updateStopTransactionInSQLite(transactionId, stopTime);
			...
			else //unknownAuthorization. Pag 18
			{
				if (status==-1) //NO APARECE ESE IDTAG NI EL LOCAL LIST NI LA LOCAL CACHE
				{
					if (getConfigurationKeyIntValue("AllowOfflineTxForUnknownId"))
					{
						...
						{
							update_authorization_list(idTagText, getExpiryDateFromCurrentTime(), NULL, _CP_AUTHORIZATION_ACCEPTED, -1,0);
						}
			...
			while (isCurrentlyUnderTransaction(connector))
			{
				...
				if (sampledData)
				{
					...
					for (int i=0; i<numSamples && *measurands; i++)
					{
						...
						double FanSpeed=get_Fan_Speed();
						double PowerLineFrequency=get_Powerline_Frequency();
						float CurrentOffered=get_Current_Offered();

						updateSqliteChargePointFloatValue("Temperature",Temperature);
						updateSqliteChargePointFloatValue("FanSpeed",FanSpeed);
						updateSqliteChargePointFloatValue("PowerLineFrequency",PowerLineFrequency);
						updateSqliteChargePointFloatValue("Current0ffered",CurrentOffered);

						float volt=get_Voltage(connector+1);
						if (connector==0) updateSqliteChargePointFloatValue("VoltageConnector1",volt);
						if (connector==1) updateSqliteChargePointFloatValue("VoltageConnector2",volt);

						float SoC=get_State_Of_Charge(connector+1);
						if (connector==0) updateSqliteChargePointFloatValue("StateOfCharge1",SoC);
						if (connector==1) updateSqliteChargePointFloatValue("StateOfCharge2",SoC);

						float Curr=get_Current_Export(i+1);
						if (connector==0) updateSqliteChargePointFloatValue("StateOfCharge1",SoC);
						if (connector==1) updateSqliteChargePointFloatValue("StateOfCharge2",SoC);

						//Esto hay que decidir como hacerlo:
						connectorValues[connector]+=get_Current_Export(connector+1);
						/////PARA EL DISPLAY////
						...
						if (isLoggedIn(NULL))
						{
							if (isCurrentlyUnderTransaction(connector))
							{
								if (counter%3==1)
								{
									sprintf(msg,"Voltage: %3.2lf", get_Voltage(connector+1));
									sendToDisplay(msg);
								}
								if (counter%3==2)
								{
									sprintf(msg,"Current Delivered: %3.2lf",get_Current_Export(connector+1));
									sendToDisplay(msg);
								}

								if (counter%3==0)
								{
									sprintf(msg,"State of Charge: %3.2lf",get_State_Of_Charge(connector+1));
									sendToDisplay(msg);
								}
							}
						}
						...
						else if (strcmp(*measurands,"Frequency")==0)
						{
							...
							sv.unit=_MV_UNIT_HZ;
							ftoa(PowerLineFrequency, sv.value, 2);
						}
						...
						else if (strcmp(*measurands,"Current.Offered")==0)
						{
							...
							sv.unit=_MV_UNIT_KWH;
							ftoa(CurrentOffered, sv.value, 2);

							//ESTO NO SE SI NO LO TENDRAN QUE HACER NAHUM:
							updateCurrentDuringTransactionInSQLite(transactionId, CurrentOffered);
						}
						...
						else if (strcmp(*measurands,"Current.Export")==0)
						{
							...
							sv.unit=_MV_UNIT_KWH;
							ftoa(get_Current_Export(connector), sv.value, 2);
						}
...
//sends  a  StartTransaction  message  and  detects  a  failure  to  process  the  message  in  the  Central  System.
...
//transaction-related message, if there is any.
	if (attempts>0 && interval>0 && attemptsTried<attempts && connectorId>=0)
	{
		...
		{
			if (debug) addLog("\n StartTransaction attempts tried es %s de 3 y OK received[connectorId-1] es %s.", convert(attemptsTried), convert(OKReceived[connectorId-1]), LOG_DEBUG, ANY);

			//Sigue fallando, volvemos a preparar el mensaje...
			queue_node *nuevo_mensaje=create_empty_queue_node(START_TRANSACTION);

			//...pero actualizamos el UniqueId
...
{
	queue_node *nuevo_mensaje;
	//char *copiaDelIdTag=strdup(idTag);

	//Pedimos autorizacion de inicio de la transaccion. Lo mira en la authentication list y si no, al Sistema Central
	//Si lo pide al sistema central NO enviamos aun el startTransaction. Se hará después.
	if (authorize(idTag))
	{
	//	if (debug) printf("\nNOS HA AUTORIZADO!!!!");
	...
	if (nuevo_mensaje)
	{
		//En caso de tener que pedir autorizacion al sistema central, se crea un nuevo hilo que lo intenta varias veces:
		pthread_t pid;
		struct lws* wsi;
	...
	{
		//En este caso es que ese IdTag esta en local y esta Rejected
		sendToDisplay("Su tarjeta ha sido rechazada. Pongase en contacto con el soporte tecnico");
	}
...
void send_stoptransaction_request(int meterStopValue, int transactionId, int reason, char *idTag)
{
	char *currentTime=getCurrentTime();
		...
		int connector=getConnectorFromTransaction(transactionId);
		int numSamplesCK=getConfigurationKeyIntValue("MeterValuesSampledDataMaxLength");
		int numSamples=-1;
...
//Pag 49: In  the  case  the ChangeAvailability.req  contains  ConnectorId  =  0,  the  status  change applies to the Charge Point and all Connectors.
	...
	else
	{
		...
		{
			unavailabilityScheduled[connector-1]=1;
			//tasksAfterTransaction(set status to avail) <-- NOT IMPLEMENTED
			response=_CP_AVAILABILITY_SCHEDULED;
		}
		else
...
//  In case of failure to set the configuration, the Charge	Point SHALL respond with status Rejected. <-- NOT IMPLEMENTED
	...
	if (isSQLiteConnected())
	{
		updateSQLiteConfigurationKey(key, value);
	}

	//En location obtenemos la posicion del array donde se encontrará la configuration Key
	...
	if (response==_CC_ACCEPTED &&getConfigurationKeyRequiresReboot(location))
	{
		...
		//Podríamos pensar que debemos rebootar aqui, pero en LA PAG 96 dice que el CP no se reiniciará por sí solo.
		//Como ya se ha actualziado la BD SQLite y cuando reboote va a coger la configuracion de dicho SQLite, no hacemos mas. Si el Sistema central
		//queire rebootar, que mande una solicitud "reset"
...
char *manageGetConfigurationRequest(const char *uniqueId_str, json_object *obj_Payload)
{
    ...
    if (num==0)
    {
    	...
    	addLog("GET-CONFIGURATION REQUEST OF ALL CONFIGURATION KEYS IN CHARGEPOINT A MAXIMUM OF %s KEY WILL BE SENT. ", convert(num_keys), NULL,LOG_INFO, ANY);
    ...
    else
    {
 	...
 	addLog("GET-CONFIGURATION REQUEST OF THE FOLLOWING KEYS: %s. ", json_object_to_json_string(obj_key), NULL,LOG_INFO, ANY);
		...
		const char *valid_from, const char *valid_to, json_object *obj_chargingSchedule)
{
	...
	if (!getConfigurationKeyIntValue("AuthorizeRemoteTxRequests"))
	{
		...
		//Esto comprueba si el conector esta o no, en estado de error.
...
//			if (debug) printf("\nSe rechaza porque el connector dio error");
	...
	if (status==_RSSS_ACCEPTED)
	{
		...
		else
		{
			...
			//Enviamos el mensaje Start Transaction tal y como se indica en la pag 53
			send_starttransaction_request(connectorId, getCurrentMeterValueOfConnector(connectorId), idTag, -1);

			//NOTA: En este caso no es necesario meter el inicio de la transaccion a SQLITE porque éste se meterá ante la respuesta del mensaje StartTransaction
		}
	}
...
char *respondRemoteStopTransactionRequest(const char *UniqueId_str, int transactionId)
{
	...
	if (connectorId>=0)
	{
		...
		changeConnectorStatus(connectorId, _CP_STATUS_FINISHING); //<-- C6 on Pag 42
				...
				else
				{
					...
					if (status==_CP_STATUS_AVAILABLE)
					{
						  if (addReservation(connectorId, expiryDate, idTag, reservationId, parentIdTag))
						  {
							  ...
							  sprintf(msg, "Conector %d reservado para el usuario %s hasta %s", connectorId+1, idTag, expiryDate);
							  sendToDisplay(msg);
						  }
						  else
						  {
...
//		booted. If any transaction is in progress it SHALL be terminated normally, before the reset, as in		Stop		Transaction		.
	...
	//Paramos todas las transacciones:
		for (int i=0; i<NUM_CONNECTORS; i++)
		{
			//Ponemos cada conector a UNAVAILABLE porque vamos a resetear el dispositivo
			changeConnectorStatus(i, _CP_STATUS_UNAVAILABLE);
			...
			if (transactions[i])
			{
				...
				if (connectorId>0)
				{
					...
					stopTransaction(connectorId-1);

					//ESTO LO QUITAMOS PORQUE YA NO TENEMOS GUI
					...
					if (strcmp(resetType_str, "Hard")==0)
					{
					...
					{
					//	changeCharpointStatus(_CP_STATUS_FAULTED)
						currentChargePointState=_CP_STATUS_FAULTED;
						currentChargePointErrorStatus=_CP_ERROR_CONNECTORLOCKFAILURE;
					}
				}
...
//		Before rebooting the settings should be store in disk or Database
		...
		if (strcmp(resetType_str, "Soft")==0)
		{
			sendToDisplay("Initializing...");

		//	int error=CP_initialize();
			if (execv(original_argv[0], original_argv))
			{
				currentChargePointState=_CP_STATUS_FAULTED;
...
char *manageSendLocalListRequest(const char *UniqueId_str, int version, json_object *obj_localAuthorizationList, const char *updateType_str)
{
	...
	int flag=0;
	//Ver Pag 77. Si nos llega una actualizacion completa...
	if (strcmp(updateType_str, UpdateType_texts[_UT_FULL])==0)
	{
		//Hay que sustituirla. Primero la eliminamos:
		emptyAuthorizationList();

		//..y luego añadimos una nueva lista. Para ello primero extraemos el tamaño de la nueva lista (del objeto JSON)
		...
		for (int i=0; i<num; i++)
		{
			...
			if (obj_idTagInfo) //Este campo es optional
			{
				...
				//Ya tenemos los cuatro campos extraidos en las variables idTag,expiryDate,parentIdTag y status_str.

				//Si flag es >0 es que hay alguna razon que nos impide actualizar la lista:
				if (!presentConfigurationCSLValue("SupportedFeatureProfiles", "LocalAuthListManagement")) flag=2;
				if (!isAuthorizationListEnabled()) flag=1;

				//...y si no hay ninguna razón, añadimos los elementos de la lista uno a uno
				if (flag==0)
				{
					//Primero Añadimos la entrada a la estructura interna
		...
		//Si hay lista, actualizamos la version
		if (num>0)
		{
			//Actualizamos la version local con lo que nos ha llegado
			localListVersion=version;

			if (isSQLiteConnected())
			{
				//Borramos toda la lista en SQLite y la añadimos entera de nuevo
				writeAuthListToSQLite();

				//Actualizamos la version local con lo que nos ha llegado
				updateSQLiteChargePointValue("AuthorizationListVersion",convert(version));
			}
		}


	}
	else //Nos llega una lista De tipo DIFFERENTIAL
	{
		//Obtenemos el numero de elementos recibidos
		int num=json_object_array_length(obj_localAuthorizationList);
		...
		for (int i=0; i<num; i++)
		{
			//Extraemos el elemento i del array recibido
			authorizationDataElement=json_object_array_get_idx(obj_localAuthorizationList, i);

			//...y de este extraemos los objetos JSON que contienen el idTag y el idTagInfo
			...
			if (obj_idTagInfo) //Este campo es optional
			{
				...
				if (flag==0)
				{
					flag=update_authorization_list(idTag, expiryDate, parentIdTag, status,-1,0);
					if (isSQLiteConnected()) insertSQLiteAuthorizationListEntry(idTag,status-1,expiryDate,parentIdTag);
				}
			}
		}
		...
		if (num>0)
		{
			//Actualizamos la version local con lo que nos ha llegado
			localListVersion=version;
			if (isSQLiteConnected()) updateSQLiteChargePointValue("AuthorizationListVersion",convert(version));
		}
	}
	...
	if (flag==0) write_list_to_disk();

	return prepare_sendlocallist_response(UniqueId_str, flag);
}
...
char *manageTriggerMessageRequest(const char *UniqueId_str, int connector, const char *MessageId_Str)
{
	...
	{
		send_bootNotification_request(getVendor(), getModel(),getChargeBoxSN(), getChargePointSN(),getFirmwareVersion(), getChargePointICCID(),
				getChargePointIMSI(), getChargePointMeterSerialNumber(), getChargePointMeterType());

		status=_TMS_ACCEPTED;
	}
	else if (strcmp(MessageId_Str, MessageTrigger_texts[_MT_DIAGNOSTICSSTATUSNOTIFICATION])==0)
	{
		if (busyUploadingDiagnosticsFile)
		{
			send_diagnosticsstatus_request(lastDiagnosticsFileStatus);
		}
		else
		{
			//Pag 36: The chargepoint shall only send status Idle after recepit of a trigger message for diagnostics file when it is not busy uploading diagnostics.
			send_diagnosticsstatus_request(_DSN_IDLE);
		}
		status=_TMS_ACCEPTED;
	}
...
{
	//lockCable(connector);
	//Ponemos el conector a charging
	...
	EnablePermissions(connector);

	//updates GUI. Actualiza el combo con los conectores que estan bajo transaccion. LO QUITAMOS PORQUE YA NO TENEMOS GUI
